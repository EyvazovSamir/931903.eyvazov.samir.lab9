...
{
	if (blackNumb === "Infinity") {alert('Число не может быть приписано к бесконечности')}
	else 
	{
		if (x == 10) 
		{
		...
		if (blackNumb == "" && x == '.') {alert("Число не может начинаться с точки");}
		else if (pointerCheck == '.' && x == '.') {alert("В числе не может быть больше одной точки");}
		else 
		{
			sdvig +=16;
...
function operButton(x)
{
	...
	else if (numbOfOper==0 && blackNumb.slice(-1) != '.') {alert("Введите число, а затем выберите нужный оператор")}
	else {alert("После выбора оператора необходимо ввести число")}
}

function equationButton()
{
	if (blackNumb == "" || blackNumb.slice(-1) === '.') {alert("Требуется закончить выражение")}
	else if (grayNumb === ""){}
	else 
	{
